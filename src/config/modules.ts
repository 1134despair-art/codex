...
const configs: ModuleConfig[] = [
  ...
  createModule({
    ...
    fields: [textField('name', '船名', true), dynamicSelectField('deviceType', '设备类型', 'accessible-device-types', true), dynamicSelectField('deviceModel', '设备型号', 'accessible-device-models', true), dynamicSelectField('deviceSN', '设备 SN', 'accessible-devices', true), { ...textField('deviceName', '设备名称'), readonly: true }, { ...textField('specification', '产品规格'), readonly: true }, textField('shipOwner', '船东姓名', true), { ...textField('customerPhone', '客户电话'), requiredOnCreate: true }, textField('customerEmail', '客户邮箱'), textField('shipLength', '船长'), { ...textField('factoryRegion', '出厂地区', true), readonly: true }, textField('installationRegion', '实际安装地区', true), { ...textField('owner', '所属经销商', true), readonly: true }, { field: 'dealerWarrantyUntil', label: '经销商设定质保到期日', type: 'date' }, { field: 'warrantyUntil', label: '当前质保到期日', type: 'date', readonly: true }, textareaField('summary', '项目备注')],
  ...
  createModule({
    key: 'devices', route: 'devices', title: '设备管理', eyebrow: '设备与产品', description: '仅展示已分发给经销商的设备；设备入库统一在仓库设备管理办理。', icon: 'cpu', crud: 'managed', permission: 'devices:view', primaryLabel: undefined,
    ...
    filters: [textFilter('code', '设备序列号', '输入完整 SN', ['code'], true), { field: 'deviceModel', label: '设备型号', type: 'select', optionSource: 'listed-device-models' }, selectFilter('country', '销售国家', countries), textFilter('region', '销售地区', '输入省、市或区域', ['region']), { field: 'ownerId', label: '所属经销商', type: 'select', optionSource: 'dealers' }, selectFilter('activation', '激活状态', [{ label: '未激活', value: 'inactive' }, { label: '已激活', value: 'activated' }]), selectFilter('bindingStatus', '绑定状态', [{ label: '未绑定', value: 'unbound' }, { label: '已绑定', value: 'bound' }])],
  ...
  createModule({
    key: 'product-catalog', route: 'product-catalog', title: '产品与型号', eyebrow: '设备与产品', description: '维护产品名称、型号、规格、终端零售价和一级经销商价；两种价格的显示名称可调整。', icon: 'boxes', permission: 'product-catalog:view', primaryLabel: '新增产品型号',
    ...
    columns: [codeColumn, { field: 'name', label: '产品名称', minWidth: 160, type: 'main' }, { field: 'deviceType', label: '设备类型', minWidth: 150 }, { field: 'deviceModel', label: '设备型号', width: 130, type: 'mono' }, { field: 'specification', label: '规格', minWidth: 170 }, { field: 'retailPrice', label: '终端零售价', width: 120, type: 'money' }, { field: 'tier1Price', label: '一级经销商价', width: 130, type: 'money' }, dateColumn('updatedAt', '更新时间'), statusColumn],
    fields: [textField('code', '产品编号', true), textField('name', '产品名称', true), textField('deviceType', '设备类型', true), textField('deviceModel', '设备型号', true), textField('specification', '规格', true), { ...numberField('retailPrice', '终端零售价（元）', true), min: 0.01 }, textField('retailPriceName', '终端零售价显示名称'), { ...numberField('tier1Price', '一级经销商价（元）', true), min: 0.01 }, textField('tier1PriceName', '一级经销商价显示名称'), selectField('status', '状态', accountStatusOptions, true), textareaField('summary', '产品说明')],
    detailTabs: [overviewTab, { key: 'current-prices', label: '当前价格', kind: 'table', source: 'product-prices', columns: [{ field: 'priceName', label: '价格名称', minWidth: 180 }, { field: 'price', label: '金额', width: 150, type: 'money' }] }, { key: 'prices', label: '价格历史', kind: 'table', source: 'price-history', columns: [dateColumn('createdAt', '生效时间'), { field: 'priceType', label: '价格类型', width: 140 }, { field: 'oldPrice', label: '原价格', width: 120, type: 'money' }, { field: 'newPrice', label: '新价格', width: 120, type: 'money' }, { field: 'operator', label: '操作人', width: 120 }] }, historyTab],
    rowActions: ['detail', 'edit', 'toggle'], actions: commonActions.filter((item) => item.key === 'toggle'),
  }),
  createModule({
    key: 'warehouses', route: 'warehouses', title: '仓库 / 仓位管理', eyebrow: '仓储与物料', description: '在一个入口查询仓库主档及其仓位；仓库、仓位仅允许新增、编辑和停用，不提供删除。', icon: 'warehouse', permission: 'warehouses:view', primaryLabel: '新增仓库',
  ...
  createModule({
    key: 'warehouse-locations', route: 'warehouse-locations', title: '库位管理', eyebrow: '仓储与物料', description: '维护仓库内的库位编码和容量，库位必须归属于一个有效仓库。', icon: 'map-pin', permission: 'warehouse-locations:view', primaryLabel: '新增库位',
  ...
  createModule({
    key: 'warehouse', route: 'warehouse', title: '仓库设备管理', eyebrow: '仓储与物料', description: '管理在库设备、出库、调货审批和设备归属变更。', icon: 'warehouse', permission: 'warehouse:view', crud: 'workflow', primaryLabel: '设备入库', primaryByTab: { stock: '设备入库', outbound: '创建出库单', transfer: '发起调货申请', ownership: undefined },
    tabs: [{ key: 'stock', label: '设备入库', field: 'category', value: '在库' }, { key: 'outbound', label: '设备出库', field: 'category', value: '出库' }, { key: 'transfer', label: '库存调货', field: 'category', value: '调货' }, { key: 'ownership', label: '归属查询', source: 'ownership-history' }],
    ...
    tabFilters: {
      stock: [{ field: 'deviceType', label: '设备类型', type: 'select', optionSource: 'stock-device-types' }, { field: 'deviceModel', label: '设备型号', type: 'select', optionSource: 'stock-device-models' }, textFilter('deviceSN', '设备 SN', '输入完整或部分 SN', ['deviceSN']), dateFilter('inboundAt', '入库时间')],
  ...
  createModule({
    ...
    tabs: [{ key: 'all', label: '全部报修' }, { key: 'pending', label: '待处理', field: 'status', value: 'pending' }, { key: 'processing', label: '处理中', field: 'status', value: 'processing' }, { key: 'completed', label: '处理完成', field: 'status', value: 'completed' }],
    ...
    columns: [{ field: 'code', label: '报修单号', width: 160, type: 'mono' }, { field: 'account', label: '用户账号', minWidth: 160 }, deviceSNColumn, deviceTypeColumn, deviceModelColumn, { field: 'faultCategory', label: '故障分类', width: 110 }, { field: 'dealer', label: '代理归属', minWidth: 170 }, { field: 'contact', label: '联系方式', width: 140 }, namedStatusColumn('处理状态'), { field: 'userConfirmationLabel', label: '用户确认', width: 120 }, dateColumn('createdAt', '提交时间')],
    ...
    rowActions: ['detail', 'assign', 'reply', 'escalate', 'complete', 'record-bill'], actions: [action('assign', '分配处理', '工单将进入处理中并通知指定经销商。', [dynamicSelectField('assigneeId', '处理经销商', 'dealers', true), textareaField('reason', '分配说明', true)], ['pending']), action('reply', '回复用户', '回复内容将保存到工单并生成通知记录。', [textareaField('replyContent', '回复内容', true)], ['processing', 'completed']), action('escalate', '转回总部', '工单归属转回总部售后，并保留流转原因。', [reasonField], ['pending', 'processing'], 'danger'), action('complete', '处理完成，待用户确认', '保存处理结果并通知用户确认；后台处理完成不等于用户已确认。', [textareaField('result', '处理结果', true)], ['processing']), action('record-bill', '生成维修支付订单', '按人工费、物料费和调整金额生成待支付订单；普通用户仅通过订单对应的二维码付款，后续由财务核实截图。', [{ ...numberField('laborHours', '人工工时', true), min: 0.5 }, { ...numberField('laborUnitPrice', '人工单价（元）', true), min: 0 }, { ...numberField('materialAmount', '物料费用（元）'), min: 0 }, { ...numberField('adjustment', '调整金额（元）'), min: -999999 }, textareaField('billingNote', '账单说明')], ['completed'])],
  }),
  createModule({
  ...
  createModule({
    key: 'materials', route: 'materials', title: '售后物料采购', eyebrow: '采购管理', description: '处理售后物料申请、审批、发货和签收；产品采购使用独立入口。', icon: 'package-check', permission: 'materials:view', crud: 'workflow', primaryLabel: '发起售后物料申请', primaryByTab: { all: '发起售后物料申请', approval: undefined, early: '发起提前申请', purchase: '发起产品采购申请', shipping: undefined, completed: undefined, rejected: undefined },
  ...
  createModule({
    key: 'material-catalog', route: 'material-catalog', title: '物料目录与库存', eyebrow: '采购管理', description: '维护物料信息、采购价、库存和启用状态。', icon: 'boxes', permission: 'material-catalog:view', primaryByTab: { all: '新增物料', low: undefined, disabled: undefined },
  ...
  createModule({
    key: 'couriers', route: 'couriers', title: '物流配置', eyebrow: '采购管理', description: '配置快递公司、快递100编码和查询凭证，并执行本地模拟测试。', icon: 'truck', permission: 'couriers:view', primaryLabel: '新增快递公司',
  ...
  createModule({
    key: 'sn-replacement', route: 'sn-replacement', title: '维修物料替换', eyebrow: '服务与售后', description: '记录维修替换前后的设备 SN、名称、型号与类型，校验归属后完成替换。', icon: 'refresh-cw', permission: 'sn-replacement:view', crud: 'workflow', primaryLabel: '发起维修替换', primaryByTab: { records: '发起维修替换', pending: undefined, exceptions: undefined },
    tabs: [{ key: 'records', label: '维修替换记录' }, { key: 'pending', label: '待确认', field: 'status', value: 'pending' }, { key: 'exceptions', label: '异常记录', field: 'status', value: 'failed' }],
    ...
    filters: [textFilter('keyword', '替换查询', '原/新 SN、设备名称或型号', ['originalSN', 'newSN', 'replacementDeviceName', 'replacementDeviceModel', 'replacementDeviceType']), selectFilter('status', '处理状态', workflowStatusOptions)],
    rowActions: ['detail', 'process'], actions: [action('process', '维修替换确认', '原 SN 将追加 -1，新设备按申请中的名称、型号和类型接续原用户、项目与经销商关系。', [reasonField], ['pending'], 'danger')],
  }),
  createModule({
  ...
  createModule({
    ...
    fields: [{ ...textField('locationSource', '地区信息来源'), readonly: true }], detailTabs: [overviewTab, { key: 'history', label: '处理记录', kind: 'timeline', source: 'workflow-events' }], rowActions: ['detail', 'resolve'], actions: [action('resolve', '处理异常', '选择调整销售地区时，将同步更新设备销售地区并保留原值与处理记录。', [selectField('resolution', '处理结论', [{ label: '确认跨区使用', value: 'confirmed' }, { label: '定位误差', value: 'location_error' }, { label: '已调整销售区域', value: 'region_adjusted' }], true), textareaField('reason', '处理说明', true)], ['pending'])],
  }),
  createModule({
    key: 'warranty', route: 'warranty', title: '质保政策配置', eyebrow: '售后配置', description: '按经销商、市场和产品配置质保时长、生效范围与起算点。', icon: 'shield-check', permission: 'warranty:view', primaryLabel: '新增质保规则',
  ...
  createModule({
    key: 'approval-center', route: 'approval-center', title: '审批中心', eyebrow: '审批中心', description: '集中查看当前账号有权处理的业务审批，待办与原业务单、当前节点和操作账号严格关联。', icon: 'circle-check-big', permission: 'approval-center:view', crud: 'readonly', primaryLabel: undefined,
  ...
  createModule({
    key: 'approval-flow', route: 'approval-flow', title: '审批人员配置', eyebrow: '售后配置', description: '业务流程与适用菜单为系统固定模板，仅允许配置审核层级和审核人员，配置仅影响新发起的业务。', icon: 'workflow', permission: 'approval-flow:view', primaryLabel: undefined,
  ...
  createModule({
    key: 'payments', route: 'payments', title: '支付账单', eyebrow: '财务与支付', description: '母账单汇总订单总额与累计付款；每次二维码付款经财务核实后生成独立子付款单，可支持同一订单分多次付款。', icon: 'receipt-text', permission: 'payments:view', crud: 'workflow', primaryLabel: undefined,
    tabs: [{ key: 'all', label: '全部母账单' }, { key: 'pending', label: '待付款/继续付款', field: 'status', value: 'pending' }, { key: 'verifying', label: '待财务核实', field: 'status', value: 'verifying' }, { key: 'verified', label: '已付清', field: 'status', value: 'verified' }],
    filters: [textFilter('keyword', '账单查询', '母订单号、子付款单号、账号、业务单号或费用项目', ['code', 'parentOrderCode', 'childOrderCodes', 'account', 'subjectCode', 'name']), selectFilter('status', '账单状态', [{ label: '待付款/继续付款', value: 'pending' }, { label: '待财务核实', value: 'verifying' }, { label: '已付清', value: 'verified' }]), dateFilter('paidAt', '最近付款时间')],
    columns: [{ field: 'parentOrderCode', label: '母订单号', width: 190, type: 'mono' }, { field: 'account', label: '用户/经销商', minWidth: 160 }, { field: 'name', label: '费用项目', minWidth: 180 }, { field: 'subjectCode', label: '关联业务单号', width: 170, type: 'mono' }, { field: 'orderAmount', label: '订单总额', width: 110, type: 'money' }, { field: 'paymentCount', label: '付款期数', width: 90, type: 'number' }, { field: 'lastChildOrderCode', label: '最近子付款单号', width: 220, type: 'mono' }, { field: 'paidAmount', label: '累计已核实', width: 120, type: 'money' }, { field: 'remainingAmount', label: '剩余待付', width: 110, type: 'money' }, statusColumn],
    fields: [{ ...textField('paymentQrCode', '订单收款码编号'), readonly: true }, { ...textField('paymentQrAccountName', '收款户名'), readonly: true }],
    detailTabs: [{ ...overviewTab, label: '母账单信息' }, { key: 'transactions', label: '子付款单', kind: 'table', source: 'payment-transactions', columns: [{ field: 'childOrderCode', label: '子付款单号', width: 220, type: 'mono' }, { field: 'installmentLabel', label: '付款期次', width: 110 }, dateColumn('paidAt', '付款时间'), { field: 'paymentQrCode', label: '收款码编号', width: 150, type: 'mono' }, { field: 'orderAmount', label: '订单总额', width: 110, type: 'money' }, { field: 'previousPaidAmount', label: '付款前累计已付', width: 135, type: 'money' }, { field: 'currentPaymentAmount', label: '本次付款', width: 110, type: 'money' }, { field: 'paidAmountAfter', label: '付款后累计已付', width: 135, type: 'money' }, { field: 'remainingAmountAfter', label: '剩余待付', width: 110, type: 'money' }, { field: 'paymentProof', label: '付款截图', width: 110, type: 'image' }, { field: 'paymentReference', label: '支付凭证号', minWidth: 175, type: 'mono' }, { field: 'operator', label: '核实人', width: 110 }, statusColumn] }, { key: 'items', label: '订单明细', kind: 'table', source: 'billing-items', columns: [{ field: 'feeType', label: '费用类型', width: 120 }, { field: 'itemName', label: '费用项目', minWidth: 190 }, { field: 'quantity', label: '数量/工时', width: 110, type: 'number' }, { field: 'unitPrice', label: '单价', width: 120, type: 'money' }, { field: 'subtotal', label: '小计', width: 120, type: 'money' }, { field: 'adjustment', label: '调整金额', width: 120, type: 'money' }] }, { key: 'history', label: '核实状态记录', kind: 'timeline', source: 'workflow-events' }], rowActions: ['detail', 'finance-verify'], actions: [action('finance-verify', '核实本期付款', '核实本次二维码付款后，系统将生成独立子付款单，并同步更新母账单的累计已付、剩余待付和付款期数。', [{ ...numberField('paidAmount', '本次核实金额（元）', true), min: 0.01 }, { field: 'paymentProof', label: '付款截图', type: 'image', required: true }, textField('paymentReference', '支付凭证号', true), { field: 'paidAt', label: '付款日期', type: 'date', required: true }, textareaField('verificationNote', '财务核实说明', true)], ['pending', 'verifying'])],
  }),
  createModule({
  ...
  createModule({
    key: 'after-sales-types', route: 'after-sales-types', title: '售后服务类型', eyebrow: '售后配置', description: '配置 APP 可提交的售后类型、必填资料和响应时限。', icon: 'list-checks', permission: 'after-sales-types:view', primaryLabel: '新增售后类型',
...
const repairCompleteAction = configs.find((item) => item.route === 'repairs')?.actions?.find((item) => item.key === 'complete')
if (repairCompleteAction) {
  repairCompleteAction.label = '完成并关闭工单'
  repairCompleteAction.impact = '总部或负责经销商在处理中节点关闭工单；涉及物料的申请须全部签收。'
}
materialConfig.fields?.splice(4, 0, dynamicSelectField('repairId', '关联维修工单（换料时选择）', 'repair-orders'))
const expenseTab = materialConfig.detailTabs?.find((tab) => tab.key === 'expenses')
if (expenseTab) expenseTab.columns = expenseTab.columns?.filter((column) => column.field !== 'warehouseDecisionLabel')
materialConfig.actions?.push(action('complete-production', '确认生产完成', '生产完成后才能进入财务核实，并为采购母订单生成首期待付款子单。', [{ field: 'productionCompletedAt', label: '生产完成日期', type: 'date', required: true }, textareaField('reason', '生产完成说明', true)], ['approved']))
const startProductionAction = materialConfig.actions?.find((item) => item.key === 'start-production')
if (startProductionAction) startProductionAction.impact = '记录采购单已导入生产计划；生产完成须单独确认，确认前不能进入财务核实。'
const financeAction = materialConfig.actions?.find((item) => item.key === 'finance-confirm')
if (financeAction) {
  financeAction.impact = '核实本次二维码付款截图和金额，按采购母订单累计已付与待付。全部款项核实后才进入仓库发货。'
  financeAction.fields = financeAction.fields?.filter((field) => field.field !== 'warehouseDecision')
}
const productWorkflowActions = materialConfig.actions?.filter((item) => item.key !== 'ship') || []
const completeReplacementAction = configs.find((item) => item.route === 'issuance')?.actions?.find((item) => item.key === 'complete-replacement')
if (completeReplacementAction) {
  completeReplacementAction.allowedStatuses = ['received']
  completeReplacementAction.impact = '物料签收后选择关联维修工单，登记旧件、新件和原因；完成后同步关闭原物料申请。'
}
...
const productPurchaseConfig = createModule({
  key: 'product-purchase', route: 'product-purchase', title: '产品采购', eyebrow: '采购与销售',
  description: '独立管理产品采购申请、销售确认、研发确认、导入生产、生产完成、财务核实、仓库发货和采购方收货闭环。',
  ...
  tabs: [
    { key: 'all', label: '全部订单', source: 'materials', field: 'category', value: '设备采购' },
    { key: 'approval', label: '销售订单审批', source: 'materials', field: 'purchaseStage', values: ['sales_confirmation', 'rd_confirmation'] },
    { key: 'production', label: '生产处理', source: 'materials', field: 'purchaseStage', values: ['production', 'production_in_progress'] },
  ...
  filters: [
    ...
    selectFilter('purchaseStage', '采购节点', [{ label: '待销售确认', value: 'sales_confirmation' }, { label: '待研发确认', value: 'rd_confirmation' }, { label: '待导入生产', value: 'production' }, { label: '生产中', value: 'production_in_progress' }, { label: '待财务核实', value: 'finance_confirmation' }, { label: '待仓库发货', value: 'warehouse_fulfillment' }, { label: '待确认收货', value: 'shipped' }, { label: '已完成', value: 'received' }, { label: '已拒绝', value: 'rejected' }]),
  ...
  rowActions: ['detail', 'approve', 'reject', 'start-production', 'complete-production', 'finance-confirm', 'purchase-ship', 'confirm-purchase-receipt'],
  actions: productWorkflowActions,
})

configs.push(productPurchaseConfig)
configs.push(createModule({
  key: 'contracts', route: 'contracts', title: '采购合同', eyebrow: '采购管理',
  description: '按采购单查询合同状态、编号、签约主体与付款进度；合同信息随采购财务节点维护。',
  icon: 'scroll-text', permission: 'contracts:view', crud: 'readonly', primaryLabel: undefined,
  tabs: [
    { key: 'all', label: '全部采购合同', source: 'materials', field: 'category', value: '设备采购' },
    { key: 'pending', label: '待确认', source: 'materials', field: 'contractStatus', value: '待确认' },
    { key: 'signed', label: '已签订', source: 'materials', field: 'contractStatus', value: '已签订' },
    { key: 'exempt', label: '无需合同', source: 'materials', field: 'contractStatus', value: '无需合同' },
  ],
  filters: [textFilter('keyword', '合同查询', '采购单号、经销商或合同编号', ['code', 'dealer', 'contractNo']), selectFilter('contractStatus', '合同状态', [{ label: '待确认', value: '待确认' }, { label: '已签订', value: '已签订' }, { label: '无需合同', value: '无需合同' }])],
  columns: [{ field: 'code', label: '采购单号', width: 180, type: 'mono' }, { field: 'dealer', label: '采购经销商', minWidth: 175 }, { field: 'contractStatus', label: '合同状态', width: 120 }, { field: 'contractNo', label: '合同编号', minWidth: 180, type: 'mono' }, { field: 'orderAmount', label: '采购金额', width: 120, type: 'money' }, { field: 'paidAmount', label: '已核实金额', width: 120, type: 'money' }, { field: 'purchaseStageLabel', label: '采购节点', minWidth: 150 }],
  fields: [], detailTabs: productPurchaseConfig.detailTabs, rowActions: ['detail'], actions: [],
}))
...
export const navGroups: NavGroup[] = [
  ...
  { label: '审批中心', items: [{ route: 'approval-center', label: '审批中心', icon: 'circle-check-big', permission: 'approval-center:view' }] },
  ...
  { label: '采购管理', items: ['product-purchase', 'materials', 'material-catalog', 'contracts', 'purchase-shipping', 'couriers'].map((route) => ({ route, label: moduleConfigs[route].title, icon: moduleConfigs[route].icon, permission: moduleConfigs[route].permission })) },
  { label: '仓储管理', items: ['warehouses', 'warehouse'].map((route) => ({ route, label: moduleConfigs[route].title, icon: moduleConfigs[route].icon, permission: moduleConfigs[route].permission })) },
  { label: '服务与售后', items: [
    ...['repairs', 'messages', 'complaints', 'issuance', 'sn-replacement', 'service-transfer'].map((route) => ({ route, label: moduleConfigs[route].title, icon: moduleConfigs[route].icon, permission: moduleConfigs[route].permission, section: '售后业务' })),
    ...['warranty', 'after-sales-types', 'approval-flow'].map((route) => ({ route, label: moduleConfigs[route].title, icon: moduleConfigs[route].icon, permission: moduleConfigs[route].permission, section: '售后配置' })),
  ] },
  { label: '财务与支付', items: [
    { route: 'contracts', label: '采购合同', icon: 'scroll-text', permission: 'contracts:view', section: '交易管理' },
    { route: 'payments', label: '支付账单', icon: 'receipt-text', permission: 'payments:view', section: '交易管理' },
