...
const canBatchImport = computed(() => auth.session?.role === 'platform' && (props.moduleKey === 'devices' || props.moduleKey === 'material-catalog' || props.moduleKey === 'warehouse' && ['stock', 'outbound'].includes(query.tab)))
  ...
  <section class="module-page">
    ...
    <el-dialog v-model="importOpen" :title="isMaterialImport ? '批量导入物料' : isOutboundImport ? '表格批量出库' : moduleKey === 'warehouse' ? '批量设备入库' : '批量导入设备'" width="860px" align-center append-to-body class="import-dialog" @closed="resetImportDialog">
      ...
      <div class="drop-zone" :class="{ ready: activeImportRows.length }">
        <AppIcon name="file-spreadsheet" :size="32" /><strong>{{ importName || '选择 Excel 或 CSV 文件' }}</strong><p>{{ isMaterialImport ? '需要包含物料编号、名称、适用设备、采购价和库存数量五列。' : isOutboundImport ? '需要包含 SN 一列；设备必须处于平台仓库在库状态，文件内不可重复。' : '需要包含 SN、设备型号、销售地区三列；可增加仓库/机构、库位及成对填写的子物料序列号和规格。' }}</p>
