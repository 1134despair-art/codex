...
describe('V3.2 complete menu business flows', () => {
  ...
  it('creates and edits every platform-maintained entity allowed by V3.2', async () => {
    ...
    const cases = [
      ...
      { module: 'approval-flow', payload: { name: '菜单巡检平台审批', levels: '平台直接审核', platformApproverId: platformApprover, status: 'normal' } },
    ...
    for (const item of cases) {
      ...
      if (['dealers', 'material-catalog', 'couriers', 'approval-flow', 'banners', 'admins'].includes(item.module)) {
  ...
  it('runs users, devices and projects through their exact non-create operations', async () => {
    ...
    const device = (await mockService.all('devices')).data.find((item) => item.bindingStatus === 'unbound')!
    ...
    expect((await mockService.create('projects', { name: '不允许新增项目' })).code).toBe(403)
  })
  ...
  it('runs dealer-only warranty, SN replacement and after-sales transfer flows', async () => {
    ...
    const replacement = await mockService.create('sn-replacement', { originalSN: original.code, newSN: 'MENU-SN-20260812' })
    ...
    expect(useDatabaseStore().records('devices').some((item) => item.code === 'MENU-SN-20260812')).toBe(true)
    ...
    const transfer = await mockService.create('service-transfer', { deviceSN: transferDevice.code, targetDealerId: target.value })
  ...
  it('rejects missing required fields and unsupported create operations at the service boundary', async () => {
    ...
    for (const module of ['users', 'projects', 'repairs', 'messages', 'complaints', 'issuance', 'payments', 'roles', 'logs']) {
